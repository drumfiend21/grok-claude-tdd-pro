...
### Joint-design regime (operator-invoked amendment to the prime directive, 2026-07-11)

The operator has explicitly invoked a **joint-design regime** in which boundary-contract design and cross-plugin fixes span both `grok-claude-tdd-pro` (GCTP) and `claude-tdd-pro` (CTP) within one process for coherence. Under this regime, prime-directive invariant 1 ("no cross-repo edits") is scope-carved as follows — invariants 2, 3, and 4 remain fully in force:

- **Working directories are separate.** GCTP work happens at `/Users/siddharthjoshi/Code/Test-GCTP-June-Seventeeth/`. CTP work happens at `/Users/siddharthjoshi/Code/claude-tdd-pro/` (sibling clone from `https://github.com/drumfiend21/claude-tdd-pro`). Neither working tree is imported into the other.
- **Commits and pushes are separate.** All work for CTP is committed inside `/Users/siddharthjoshi/Code/claude-tdd-pro/` and pushed to CTP's remote. All work for GCTP is committed inside `/Users/siddharthjoshi/Code/Test-GCTP-June-Seventeeth/` and pushed to GCTP's remote. Never stage a CTP file in a GCTP commit; never stage a GCTP file in a CTP commit.
- **GCTP still consumes CTP by pinned reference.** The plugin cache at `.harness/plugin-cache/claude-tdd-pro/` remains the read-only pinned artifact GCTP depends on. A CTP change lands in the CTP repo → CTP tags a CL → GCTP §15-gated pin bump ADR → GCTP re-materializes cache. The pin-bump discipline (per ADR-0092/0093/0094 precedent) is NOT bypassed by the joint-design regime.
- **Contract-only coupling still holds.** Reaching into plugin internals from GCTP scripts is still forbidden. The joint-design regime affects *authorship location and review coordination*, not the runtime coupling.
- **Independent release cadence still holds.** CTP can ship without GCTP; GCTP can pin whenever appropriate.

Rationale: for boundary contracts (like P-15 acquisition, P-18 sufficiency, P-19/P-20/P-21 investigations), both sides of the seam benefit from one designer reasoning about both surfaces. Reverting to strict cross-repo-forbidden after each session-end is fine; the joint-design regime is a session-level operating mode.

Rescission: the operator can revert to strict prime-directive-only mode at any time by stating so; sessions started without an explicit joint-design invocation default to the strict mode.
