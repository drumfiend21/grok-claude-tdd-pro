...
- `docs/agent-operating-compact.md` — the **agent operating compact**: the behavioral binding on whatever agent drives GCTP (per TICKET-068 / ADR-0057). You act only as the *user* of GCTP — drive only `/consult → /roadmap → /decompose → /dispatch → /inner-loop → /audit`; never architect the user's product yourself; never call CTP's engine directly or hand-write req/res/architecture artifacts; nothing enters the app repo that GCTP didn't generate. Honest caveat: GCTP is command-prompts + scripts that *you* execute, so your cognition (the CTP→plain-language translation `/consult` instructs) is unavoidable — but it is confined to executing the procedure and translating CTP's output, never to originating architecture. Scope: this governs building the user's product *through GCTP*, NOT harness self-maintenance (which runs on the ADR/founder-directives/R-G-C/TDD plane). **Enforcement is fail-closed:** the operator MUST accept (`scripts/accept-compact.sh`); until a current acceptance exists at `.harness/agent-compact-ack.json`, you MUST NOT drive the sanctioned workflow for the user's product — only read docs + run the accept script. `scripts/audit-agent-compact.sh` is the machine gate (pre-commit + CI).
