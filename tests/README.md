...
| `scripts/standards-refresh.sh` | `tests/test-standards-refresh.sh` | 17 (per TICKET-075 / ADR-0064; drives CTP source-refresh on an operator cadence — `<N>m/h/d/w/mo` grammar accept/reject, configure/status/significance, cadence due/not-due/after-interval via `SR_NOW`, `--force`, on-demand manual, missing-entrypoint non-fatal; exit 0/2) |
