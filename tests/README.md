...
| `scripts/audit-standards-enforced.sh` | `tests/test-audit-standards-enforced.sh` | 10 (per TICKET-074 / ADR-0063, "Fix C"; dynamic re-run gate — green `rules_verified` claims must equal a live detector re-run + every live pass evaluated ≥1 file; flags claim-pass-on-live-fail + vacuous-pass; vacuous on no app_root / no green response; exit 0/1/2) |
| `scripts/standards-refresh.sh` | `tests/test-standards-refresh.sh` | 19 (per TICKET-075 / ADR-0064 + ADR-0065; drives CTP source-refresh on an operator cadence — `<N>m/h/d/w/mo` grammar accept/reject, configure/status/significance, cadence due/not-due/after-interval via `SR_NOW`, `--force`, on-demand manual, missing-entrypoint non-fatal, two-layer alignment writes CTP's fetch registry; exit 0/2) |
| **Total** | **31/31 testable surfaces** | **~400 assertions** |
