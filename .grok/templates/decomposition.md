...
- [ ] `tickets[*].applicable_rules` includes EVERY EO-namespace rule (`source_namespace: eo`) in `active.json` — non-exemptible (TICKET-050 / ADR-0045). Vacuous while the EO set is empty (pin-bump-gated).
