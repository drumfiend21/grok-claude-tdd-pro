...
- **Precedent:** P-10 → CL-538 → ADR-0079 (24h cycle). P-11 → CL-541..CL-545 (§29..§29.6) → ADR-0086 at pin `0cf28fe`. P-12 → CL-546 (§30 / S-57 / §2.35) → ADR-0087 at pin `f060a8e` (24h cycle again) → CL-547 (§30.1) → ADR-0088 at pin `c23e5fe` → CL-548 (§30.2) → ADR-0089 at pin `43ea692` (all three landed by CTP in one operator cycle after successive GCTP readiness reviews). Same class, same discipline; symmetric fix one stage earlier in the pipeline (intake instead of design), with the coverage of that intake made both precise and transparent.
