...
## P-13 — Cloud identification + classification from operator answers (classifier haystack is vision-only, so cloud-agnostic visions never activate cloud probes even when operator explicitly names one)

- **Status:** 📋 FILED — 2026-07-07, at CTP pin `f39fcdc`. Awaiting a v1.15 tag from CTP (target close-out: **§30.4 — Classifier over vision + answers** as CL-550-ish). This proposal takes CTP up on its standing offer in the resume-kata handoff §4: *"If you want the classifier to fire `aws-platform` when the operator states 'AWS' in a business answer, that's a small CTP-side signal addition — tell me and I'll add it; otherwise the design-stage sourcing is correct."* Operator has explicitly requested the classifier-side fix over the design-stage sourcing alternative.
...
- **Root cause (upstream).** `commands/full-surface-intake.sh` builds the classifier haystack from `ENV["WORKLOAD"]` OR `u_answers["workload"]` (fallback), then lowercases it. All other universal answers (`motivation`, `criticality`, `rto_target`, `rpo_target`, `sensitivity`, `compliance`, `scale`, `budget`) are invisible to the classifier. Probe answers are also invisible (they arrive later anyway). This was a natural simplification when §30 shipped — the vision text is the "source of truth" for what the workload is — but it doesn't handle the case where **the operator's answer is the earliest explicit signal** for a dimension (like cloud target) that a well-formed vision legitimately omits.
- **What the proposed fix delivers.** v1.15 §30.4 changes the haystack from vision-only to vision + universal-answer values (one-line edit). Additionally (RECOMMENDED as Extension), adds a single `target_platform` universal question so operators deterministically supply cloud rather than depending on a coincidental mention in another answer. **Additivity invariant:** the haystack can only grow, so the classifier can only fire MORE types, never fewer — every v1.0 and v1.1 profile emitted before §30.4 continues to validate + translate + recommend unchanged. Word-boundary matching from §30.3 preserved (regression checked in the acceptance test).
- **Boundary contract (what crosses to GCTP).** Nothing new crosses. `business-profile.json` schema unchanged (v1.1 shape stable). If Extension ships, `answers.target_platform` becomes an additive optional key inside the existing `answers` object. `--classify` output shape unchanged. Prime-directive-preserving.
- **Full technical handoff:** [`docs/handoff-ctp-p13-cloud-classification-from-answers.md`](handoff-ctp-p13-cloud-classification-from-answers.md); machine-verifiable acceptance test at [`docs/handoff-ctp-p13-acceptance-test.sh`](handoff-ctp-p13-acceptance-test.sh) (12 assertions covering Core Fix, monotonicity, word-boundary preservation over the new haystack, multi-cloud disambiguation, undecided/on-prem/hybrid handling, vision + answer union, v1.0 back-compat, and Extension shape).
- **Coordination back to GCTP (post-adoption).** Filed as **TICKET-118** in `TICKETS.md`. Once CTP tags v1.15: §15-gated pin bump via new ADR; append a "Classifier over vision + answers (§30.4)" bullet to `docs/handoff-contract.md §Business-Intake`; no code change to `scripts/consult.sh --validate-profile` for Core Fix (v1.1 shape stable); if Extension ships, `--validate-profile` MUST NOT require `answers.target_platform` on v1.1 profiles missing it (additive-optional discipline, already GCTP's default posture per ADR-0089 precedent); `/consult` skill picks up the new universal question in its Stage-1 walk if Extension ships.
- **Precedent:** Symmetric to §30.2 (precise classification) and §30.3 (precise matching) — this is precise *input surface* for the classifier. Same discipline: additive per ADR-0047, zero deletions, monotone, prime-directive-preserving. Extends the P-12 arc from four halves closed to five.
