...
- **Status:** ✅ ADOPTED — proposed 2026-07-02; fixed upstream in **CTP CL-541..CL-545 (§29..§29.6)** and adopted in GCTP via **ADR-0086** at pin `0cf28fe` on 2026-07-03. CTP's `commands/full-surface-consult.sh` + `rubric/enforce-write-time.sh` deliver: (1) full aggregator-surface ingestion at consult time; (2) per-decision verdicts across every namespace (`needs_grounding=0`); (3) fail-closed on silent omission (`--require-complete` gate); (4) byte-identical native enforcement — the consult and write-time governor now share one primitive by construction. Both CTP return handoff docs (`handoff-ctp-to-gctp-p11-fixed.md` + `handoff-ctp-to-gctp-byte-identical-enforcement.md`) live in the CTP repo at `0cf28fe`. **GCTP-side companion TICKET-113** (strengthen `audit-architecture-crosscheck.sh` for verdict-completeness) is now unblocked and queued.
- **Original filing (2026-07-02):**
- **Found:** 2026-07-02, first-juncture consult run of `business-intake.sh` + `architect-session.sh` on the fresh kata brief. Operator observation (verbatim): *"I don't understand why the architecture that you're producing through GCTP is not using all one hundred and eighteen rules. Everything that CTP produces for GCTP must be enforced by one hundred and eighteen rules, not only the IAC rules."*
- **Symptom.** `architect-session.sh` emitted an `architecture-options.json` in which every one of the 4 options and every one of the 25 derived technical concerns (`technical-requirements.json`) was cited **only** to this grounding set:
  - `aws-reliability-pillar`
  - `aws-rpo-rto-targets`
  - `nist-800-53`
  - `google-sre-book` (SRE ops — **not** Google's TypeScript/JS/Python style guides)
  - `opentelemetry-docs`

  `active.json` at pin `a69f380` (ADR-0085) carries **118 rules across 43 namespaces** — including `ts` (8), `node` (10), `react` (10), `owasp`, `oas` (3), `jwt` (3), `web-vitals`, `w3c`, `security-governance` (EO-2026), `arch` (3), `doc` (2), `k8s` (10), `helm` (3), `sbom` (2), `sarif` (3), `iam` (4), and 26 others. **None** of these namespaces were consulted at architecture-production time. As a direct consequence, the engine cannot recommend design decisions that pre-account for the code-level rules that will bite at `/inner-loop` time — e.g., it does not tell the operator "your API layer will need `g-oas-declare-security-schemes` + `g-jwt-no-alg-none-*` + `g-oas-no-plaintext-http-server` compliance, so design an auth-gateway now" or "your frontend will need `g-react-007` (component-level test coverage), so budget for that in delivery sizing."
- **Impact on consumers.** The operator-declared standards regime (ADR-0037) requires that operator-supplied `active.json` govern *both* production and enforcement. Today, at consult time, only the IaC/reliability/SRE subset governs production; the code-level, contract-level, and prose-level namespaces flow in only at `/decompose` time via per-ticket `applicable_rules` filtering. This produces a class of *architectural rework loops*: an option looks green at consult, then at code time a rule fires that would have changed the architecture had it been surfaced earlier. It also weakens GCTP's Stage-5 cross-check (`scripts/audit-architecture-crosscheck.sh` per ADR-0056) — the cross-check today can only verify that the rules CTP *did* cite resolve and that the EO namespace is present; it cannot verify that CTP reasoned against the full 118-rule surface, because CTP does not emit that reasoning.
- **Root cause (upstream).** `commands/architect-session.sh` (and by transitivity `commands/architect-recommend.sh` and `commands/well-architected-review.sh`) has a hand-picked pillar/source list burned in at the engine layer. It does not ingest `rubric/aggregator.sh`'s output (or `active.json`, or a comparable full-surface manifest) as its grounding corpus. The CTP-side rule aggregator already builds the 118-rule surface; the CTP-side architect engines simply do not consume it.
- **Proposed upstream fix.** Extend CTP's consult engines so that, at architecture-production time, they:
  1. **Ingest the full aggregated standards surface** (the same manifest that feeds `active.json`), not a hand-picked pillar subset. CTP already builds this via `rubric/aggregator.sh`; the engines just need to consume it.
  2. **For every decision emitted**, produce a per-rule verdict across all applicable namespaces — one of `applies` (with the concrete design implication for this decision), `not_applicable` (with a one-line rationale, e.g., "no React in this workload"), or `needs_deviation` (with the shape of the deviation that would be required to proceed). The current `applicable_rules` field on the emitted architecture ADR is the natural carrier — today it holds only the small subset CTP chose to cite; the fix makes it exhaustive.
  3. **Fail-closed** when any namespace is silently omitted. If a workload is TypeScript-heavy (visible from the profile / vision), suppressing the `ts` namespace at architect time is an engine bug, not a design choice.
  4. **Emit a design-time-vs-code-time hand-off note** per decision: what will bite at `/inner-loop` under this decision, so the operator can size accordingly. This is the missing signal that lets the operator budget for `g-react-007` component tests, `g-node-002` typed errors, `g-owasp-*` app-layer controls, etc. at architecture time rather than at rework time.
  5. **Grounding for the fix.** ADR-0037 (operator-declared standards regime), ADR-0045 (EO governance as standing dimension — additive), ADR-0046 (two-phase enforcement — design phase + code phase), ADR-0047 (standards layer additive-only). The proposal composes on these; it does not weaken any existing rule or introduce a subtractive change.
- **Consumer-side companion (GCTP, NOT an upstream item, filed as `TICKET-113`):** extend `scripts/audit-architecture-crosscheck.sh` so that, once P-11 lands upstream, GCTP verifies not just (a) cited rule-ids resolve and (b) EO is present, but also (c) *every applicable namespace from `active.json` has a per-rule verdict on every decision*. Fail-closed if any namespace is silently omitted. The wire already exists (Stage 5 of ADR-0056); the strengthening becomes meaningful only once CTP emits the fuller reasoning.
- **Testability upstream.** A minimal repro: run `commands/business-intake.sh` with any TypeScript/React-flavored workload description, then `commands/architect-session.sh --profile … --vision …`, inspect `architecture-options.json[].grounding`. Today: 5 sources, all IaC/SRE. After P-11: 43 namespaces represented, each with per-rule verdict per decision.
- **Why this matters for the O'Reilly kata attempt.** GCTP's stated mission on this branch is a world-class submission that meets/exceeds the top-3 Winter 2025 finalists (ZAITects, Litmus, SAG). The published quality bar (`../softarchcert-kata-2026-07-02/docs/quality-bar.md`) documents that all three finalists shared gaps precisely in the namespaces CTP does not consult at design time — Google TS/React discipline for the code layer, OWASP ASVS for the app layer, OAS discipline for the API contract, WCAG for the candidate-facing UX, EO-2026 for regulatory compliance. Closing those gaps is exactly what would differentiate the submission. Under P-11, that reasoning becomes part of the architecture itself; without it, it can only be applied ex-post at ticket time.
