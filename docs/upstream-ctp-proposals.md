...
- **Status:** 📋 FILED — 2026-07-04 — full technical handoff at [`docs/handoff-ctp-p12-full-surface-intake.md`](handoff-ctp-p12-full-surface-intake.md). Proposed fix ships as **CTP v1.14 §27.16 — Full-Surface Intake** (workload classifier + per-namespace probe groups + additive schema bump `1.0` → `1.1`). Additive per ADR-0047; contract-stable. Awaiting CTP `v1.14`; GCTP re-pins via TICKET-114 after tag lands.
...
- **Coordination back to GCTP (after v1.14 tags).** GCTP re-pins via ADR-gated bump (ADR-0086 procedure); updates `docs/handoff-contract.md` with the extended `§Business-Intake` schema section; extends `scripts/consult.sh --validate` for schema_version ≥ 1.1 checks; extends `scripts/audit-architecture-crosscheck.sh` to verify every activated probe group's committed posture propagates into at least one `decisions[]` juncture's `applicable_rules`; updates the `/consult` skill to walk the Stage 0 → Stage 1 → Stage 2 cascade. Scoped as **TICKET-114** in `TICKETS.md` (queued, blocked on CTP release). Full detail in the handoff document.
- **Precedent:** P-10 → CL-538 → ADR-0079 (24h cycle). P-11 → CL-541..CL-545 (§29..§29.6) → ADR-0086 at pin `0cf28fe`. P-12 is the symmetric fix one stage earlier in the pipeline (intake instead of design). Same class, same discipline.
