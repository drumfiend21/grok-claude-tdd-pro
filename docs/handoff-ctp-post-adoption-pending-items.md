...
**Status:** 🟢 **CTP-SIDE PENDING ENUMERATION** — GCTP acking CTP's two post-adoption offers (production-fetch wrapper + `--explain`) and formally re-asking on the still-FILED P-14 (§30.7); GCTP-side pre-wire + E2E tests are ready to consume each item the moment it ships. No decisions blocking on GCTP.
**Prior turns:**
...
|---|---|---|---|---|
| **1** | **Production-fetch wrapper** — thin wrapper that runs `standards/fetchers/*` against umbrella-matched source URLs so `acquire-technology-rules.sh` pulls live content itself | CTP's post-adoption offer 2b (2026-07-10) | **Real KA-2 acquisition** (currently gated on `--source-file <pre-fetched>` boundary stub) | A5–A10 E2E tests queued in `tests/test-p15-acquisition.sh` (not-yet-written); TICKET-124 kata driver update queued; the moment the wrapper ships, GCTP writes A5–A10 against real Vue fetches and runs KA-2 live |
| **2** | **`--explain` mode on the four commands** — operator-friendly output instead of terse `key=value` markers on `resolve-technology.sh` / `acquire-technology-rules.sh` / `promote-project-rule.sh` / `recommend-technology.sh` | CTP's post-adoption offer (2026-07-10) | GCTP `/consult` skill plain-language translation quality (skill still does loop-level narrative; `--explain` sharpens the per-command surface) | GCTP consumes both formats — terse for machine parsing (A1–A18 assertion suite already keys on `key=value` markers in `tests/test-p15-family-activation.sh`), `--explain` for `/consult` skill translation. No schema change needed on GCTP; additive |
...
Nothing blocked on GCTP. All three items are additive per ADR-0047; item 1 unblocks the end-to-end kata acquisition path; item 2 is UX-only; item 3 is Stage-0 discoverability, orthogonal to the shipped §31 acquisition path.
