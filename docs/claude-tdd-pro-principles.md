...
These are the operational rules for consuming Claude TDD Pro in this repo. Per Musk #1 deletion-pass closure (TICKET-028 / ADR-0033), the active C-rule set is **4 harness-side rules** (C-1, C-22, C-23, C-24); 20 inner-loop discipline rules (formerly C-2..C-21) are CONSOLIDATED into the upstream plugin per the CLAUDE.md prime directive. The harness-side rules are numbered `C-N` to keep them distinct from architectural rules (`R-N`) and Grok-orchestration rules (`G-N`); every change inside acceptance-tested scope is checked against the active harness-side rules + the upstream SKILL.md trio's enforcement.
...
| ~~C-2..C-21~~ | **CONSOLIDATED — delegated to upstream plugin per Musk #1 deletion-pass closure (TICKET-028 / ADR-0033).** The 20 inner-loop TDD discipline rules (Three Laws nano-cycle; green-checkpoint commits; tdd-pro-cl-workflow invocation; "green" as full gate; refactor on green only; smells-drive-priority; characterization tests for legacy; test pyramid; TDD-school per CL; no SUT mocking; behavior-naming; coverage as leading indicator; headless `-p` contract; `--allowedTools` scoping; reference-only skill consumption; SKILL.md frontmatter activation; no skill substitution; atomic ticket=CL=commit; decision-trail emission; inner-loop no-research no-deploy) are owned by `claude-tdd-pro` per the CLAUDE.md prime directive. The harness consumes via the SKILL.md trio symlinked from the pinned plugin cache. Canonical rule bodies live in `claude-tdd-pro/.claude/skills/tdd-pro-cl-workflow/SKILL.md` + upstream `architecture-v1.9.md`. Rule-body intellectual provenance retained in §§1-15 above (Beck/Bob/Fowler/Feathers/Cohn/London-Chicago/mutation/property/Anthropic) — operational citation responsibility transferred to upstream per R-2 versioned consumption. | §§1-15 (provenance only); upstream plugin (operational) |
